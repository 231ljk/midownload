...
function Main {
    ...
    $pythonFound = $false
    try { $null = & python --version 2>&1; if ($LASTEXITCODE -eq 0) { $pythonFound = $true } } catch {}
    if (-not $pythonFound) {
        try { $null = & py --version 2>&1; if ($LASTEXITCODE -eq 0) { $pythonFound = $true } } catch {}
    }
    if (-not $pythonFound) {
        Write-Warn "未检测到 Python 3，Nx 需要 Python 3 才能运行。"
        Write-Host "请从 https://python.org 下载并安装 Python 3，"
        Write-Host "安装时请勾选 'Add Python to PATH' 选项。"
        Write-Host ""
    } else {
        Write-Success "Python 环境检测通过"
    }
    ...
    try {
        ...
        $interpreterUrl = "https://raw.githubusercontent.com/$NX_REPO/$NX_BRANCH/bin/nx_interpreter.py"
        Download-File $interpreterUrl "$NX_BIN_DIR\nx_interpreter.py"
    }
