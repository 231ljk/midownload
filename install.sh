...
install_dependencies() {
    ...
    if ! command -v python3 &> /dev/null && ! command -v python &> /dev/null; then
        ...
        case "$pkg_manager" in
            ...
            *)      error "请手动安装 Python 3 (https://python.org) 后重试"; exit 1 ;;
        esac
    fi
    info "依赖检查完成: curl + Python 3"
}
...
main() {
    ...
    if curl -fsSLI "$release_url" &> /dev/null; then
        ...
        local interpreter_url="https://raw.githubusercontent.com/${NX_REPO}/${NX_BRANCH}/bin/nx_interpreter.py"
        download_file "$interpreter_url" "$NX_BIN_DIR/nx_interpreter.py"
    fi
